...
// Validate password requirements (8+ chars with numbers)
function validatePasswordRequirements() {
  const password = document.getElementById('signupPassword');
  const value = password.value;
  
  // Check if password meets requirements: at least 8 chars and has numbers
  const isValid = value.length >= 8 && /\d/.test(value);
  
  if (value.length === 0) {
    // Empty - remove validation styles
...
// end validatePasswordRequirements
...
document.getElementById('signupForm').addEventListener('submit', function(e) {
  ...
  if (password.length < 8) {
    ...
    document.getElementById('errorPassword').textContent = 'Password must contain at least one number';
    hasError = true;
  }
  ...
  // Validate role
  if (!role) {
    document.getElementById('errorRole').textContent = 'Please select a role';
    hasError = true;
  }
  ...
  showToast('Account created successfully! Role: ' + newUser.role + '. You can now sign in.');
